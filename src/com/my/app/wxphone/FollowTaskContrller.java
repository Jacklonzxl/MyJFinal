...
//		  float f_cnt=0;
	...
	public void save() {
		...
		if(followTask!=null){
			...
			if(followTask.getInt("total_quantity")<100)
			{
				setAttr("msg", "关注数不能小于100！");
				renderJsp(root+"followmsg.jsp");
				
			}else if(!isfuckdo&&hasmoney&&(public_account_old.equals(public_account)||isokPublic_account))
			{
				followTask.update();
				renderJsp(root+"followsuccess.jsp");
			}else if(isfuckdo)
			{
		...
		}else
		{
			...
			if((day_cnt+bean.getInt("total_quantity"))<=max_day_follow)
			{
				...
				if(bean.getInt("total_quantity")<100)
				{
					setAttr("msg", "关注数不能小于100！");
					renderJsp(root+"followmsg.jsp");
					
				}else  if(isokPublic_account&&hasmoney)
	            {
	            	bean.save();
	            	renderJsp(root+"followsuccess.jsp");
	            }else if(!hasmoney)
