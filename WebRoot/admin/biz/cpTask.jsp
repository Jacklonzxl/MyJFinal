...
<div class="toolbar row">
	<div class="col-sm-6 hidden-xs">
		<div class="page-header">
			<h1>CP任务 <small>查看及管理CP任务 信息</small></h1>
		</div>
	</div>
	...
	<div class="col-md-12">
		...
		<div class="panel panel-white">
			...
			<div class="panel-body">
				...
				<option value="1" <c:if test="${param.status=='1'}">selected</c:if> >停用</option>
				<option value="0" <c:if test="${param.status=='0'}">selected</c:if> >启用</option> 
				</select> &nbsp;&nbsp;&nbsp;&nbsp;关键字:&nbsp;</label>
				...
				<tr>
					...
					<th class="hidden-xs">日推送量</th>
					<th class="hidden-xs">推送手机</th>
				...
				<tr>
					...
					<td class="hidden-xs">${list.cpid}</td>				
					<td class="hidden-xs">${list.daycnt}</td>
					<td class="hidden-xs">${list.cids}</td>
					<td class="hidden-xs"><c:if test="${list.status==0}">启用</c:if><c:if test="${list.status==1}">停用</c:if></td>
					<td class="center">
