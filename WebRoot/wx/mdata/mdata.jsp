...
<form action="#" method="post" id="form1" name="form1" target="iframe1" >
     ...
     <section style="padding-top:10px">
       ...
       <select class="select" style="height:32px;width:70px" id="status" name="status">
          ...
          <option value="1"  <c:if test="${param.status=='1'}">selected="selected"</c:if>>正常</option>
          <option value="0"  <c:if test="${param.status=='0'}">selected="selected"</c:if>>异常</option> 
      </select>
                    分组:
     ...
     <br/>
      ...
      <table class="table" id="tablelist">
       ...
       <tr> 
        ...
        <td style="text-align:center;width:150px">${list.lasttime}</td>
        <td style="text-align:center;"><c:if test="${list.btime>=40}"><span style="color:red">异常</span></c:if><c:if test="${list.btime<40}">${list.btime}分</c:if></td>
        <td style="text-align:center;">
         <a href="#" onclick="loadinput('${m_name}','${list.dnum}',1)">修改</a>
         <a href="#" onclick="loaddel('${list.dnum}')"  >删除</a> 
        </td>
       </tr>
       </c:forEach>    
      </table>
